# GsCore Frontend / gsuid_hub v0.0.19
...
| `/ai-capability-agents` | AICapabilityAgentsPage | 能力代理节点（工具包、关键词、边界）管理 |
...
`children` 跟着滚。全站 13 个页面有 `toolbar`（`/ai-knowledge`、`/ai-capability-agents`、`/database` 等），
13 个没有（`/ai-memory`、`/dashboard` 等）。
...
| [五](docs/skills/gshub-development/references/05-components-and-form-controls.md) | 表单/筛选控件规范（统一 `h-9`、Select 哨兵） |
| [六](docs/skills/gshub-development/references/06-reusable-component-catalog.md) | 封装组件目录（**必须复用，禁止手搓**） |
...
主要 API 封装位于 `src/lib/api.ts`，当前导出：

| 领域 | API 对象 |
| --- | --- |
| 概览与数据 | `dashboardApi`、`databaseApi`、`versionApi`、`brandApi` |
| 配置 | `configApi`、`frameworkConfigApi` |
| 插件 | `pluginsApi`、`pluginStoreApi`、`gitMirrorApi`、`gitUpdateApi` |
| 运维 | `logsApi`、`traceApi`、`remoteCommandApi`、`schedulerApi`、`backupApi`、`systemApi` |
...
| AI 运行 | `historyApi`、`aiSessionLogsApi`、`agentDebugApi`、`aiScheduledTasksApi`、`aiKanbanApi`、`aiApprovalsApi`、`aiStateStoreApi` |
...
- 一行筛选控件高度统一 `h-9`；复杂表单优先复用 `components/config` 下的配置组件。
...
- 语言资源按模块拆分在 `src/i18n/locales/{locale}/`（当前每种语言 45 个模块 JSON + 一个 `index.ts`）。
